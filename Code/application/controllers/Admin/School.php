...
class School extends ADMIN_Controller {
    ...
    public function update($id) {
        ...
        $this->data['user'] = select(TBL_SCHOOLS, FALSE, array('where' => array('id' => $id)), array('single' => TRUE));
        ...
        $this->form_validation->set_rules('username', 'School Name', $user_rule);
    ...
    public function blocked($id) {
        update(TBL_SCHOOLS, $id, array('user_status' => 'blocked', 'modified_date' => date('Y-m-d H:i:s', time())));
        $this->session->set_flashdata('success', 'School is Successfully Blocked.');
    ...
    public function active($id) {
        update(TBL_SCHOOLS, $id, array('user_status' => 'active', 'modified_date' => date('Y-m-d H:i:s', time())));
        $this->session->set_flashdata('success', 'School is Successfully Activated.');
        redirect($this->data['prev_url']);
    }
    ...
    public function send_message($id) {
        ...
        $this->data['page_title'] = 'School Send Message';

        if (empty($id) && !is_numeric($id)) {
            redirect('admin');
        }

        $this->data['u'] = select(TBL_SCHOOLS, FALSE, array('where' => array('id' => $id)), array('single' => true));
        $this->data['templates'] = select(TBL_MESSAGES, FALSE, array('where' => array('is_template' => '1')));
        $this->data['users'] = select(TBL_SCHOOLS, TBL_SCHOOLS . '.username,' . TBL_SCHOOLS . '.id,' . TBL_ROLES . '.role_name,' . TBL_ROLES . '.id as rid', array(
            'where' => array(TBL_SCHOOLS . '.is_delete' => FALSE),
            'where_not_in' => array(TBL_SCHOOLS . '.user_status' => array('blocked'))
                ), array(
            'order_by' => TBL_SCHOOLS . '.username',
            'join' => array(
                array(
                    'table' => 'roles',
                    'condition' => TBL_SCHOOLS . '.role_id=' . TBL_ROLES . '.id'
                )
            )
                )
        );
        if (count($this->input->post('all_users')) == 0) {
            $this->form_validation->set_rules('all_users', 'Schools', 'trim|required');
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($all_users)) {
                ...
                foreach ($all_users as $user) {
                    ...
                    $user_mail = select(TBL_SCHOOLS, 'email_id', array('where' => array('id' => $user)), array('single' => TRUE));
    ...
    public function send_messages() {
        ...
        $this->data['page_title'] = 'Schools Send Messages';
        ...
        if ($_POST) {
            ...
            if (isset($_POST['all_users'])) {
                ...
                $this->data['my_cnt'] = 0;
                $this->form_validation->set_rules('all_users[]', 'Schools', 'trim');
            }
        } else {
            $this->data['post_users'] = array();
        }
    ...
    public function activity($id) {
        ...
        $user_data = select(TBL_SCHOOLS, FALSE, array('where' => array('id' => $id)), array('single' => TRUE));
        ...
        $data['my_activities']['studymates'] = select(TBL_SCHOOLS . ' u', $select, $where, $option);
