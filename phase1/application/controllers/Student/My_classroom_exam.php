	...
	{	
		...
		if($data['verify']['is_started'] == 1){
			$data['exam_status'] = select(
					...
					'sc.id, e.exam_name, sc.exam_id,sc.created_date,IF(TIMESTAMPDIFF(SECOND,NOW(),sc.created_date + Interval e.duration minute) < 0,0,TIMESTAMPDIFF(SECOND,NOW(),sc.created_date + Interval e.duration minute)) AS remaining_time',
