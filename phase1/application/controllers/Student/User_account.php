...
class User_account extends CI_Controller {
	...
	{	
		...
		if(isset($this->session->userdata('user')['id'])){
			...
			$bdate = $user_session['birthdate'];
			$date=date_create($bdate);
			$birthdate = date_format($date,"d-m-Y");

			$this->data['full_name']	= $user_session['full_name'];
			$this->data['first_name']	= $user_session['first_name'];
			$this->data['last_name']	= $user_session['last_name'];
		...
		if(isset($this->session->userdata('user')['id'])){
			...
			if($this->input->get_post('new_password') != ''){
				$this->form_validation->set_rules('con_password', 'Confirm Password', 'trim|matches[new_password]|required');
			}
		...
		$this->form_validation->set_rules('first_name', 'First name', 'trim|required|callback_check_first_name');
		$this->form_validation->set_rules('last_name', 'Last name', 'trim|required|callback_check_last_name');
...
/*---check last name is valid or not------*/
	public function check_last_name(){
		...
		{
			$this->form_validation->set_message('check_last_name', 'Invalid Last Name');
			return FALSE;	
		}
	}
		...
		if($birthdate != ''){
			...
			if (preg_match("/^(0[1-9]|[1-2][0-9]|3[0-1])-(0[1-9]|1[0-2])-[0-9]{4}$/",$birthdate)){
				if($birthdate == '00-00-0000')
					...
					return $this->input->post('thealtdate');
			}
			else{
				$this->form_validation->set_message('check_birth_date', 'Invalid Date of Birth');
				return FALSE;
