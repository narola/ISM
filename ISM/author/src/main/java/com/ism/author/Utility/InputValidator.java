...
public class InputValidator {
    ...
    public boolean validateStringPresence(ContactsCompletionView contactsCompletionView) {
        if (contactsCompletionView.getText() == null
                || contactsCompletionView.getText().toString().trim().length() == 0) {
            setError(contactsCompletionView, context.getString(R.string.required));
            return false;
        } else {
            contactsCompletionView.setError(null);
            return true;
        }
    }
