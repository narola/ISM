...
class SocketIOConnection {
    ...
    /*
     * add by hitesh for gteting time out call back
     */
    ArrayList<TimeoutModel> timeoutModels =new ArrayList<TimeoutModel>();
      int ackCount;
//     int tmoCount;

    public void emitRaw(int type, SocketIOClient client, String message, Acknowledge acknowledge,String UniqueId,TimeoutCallback callback) {
        String ack = "";
//        String tmo = "";
        if (acknowledge != null) {
            String id = "" + ackCount++;
            ...
            TimeoutModel model =new TimeoutModel();
            model.timeoutCallback = callback;
...
//        }
            ...
            public void onCompleted(final Exception e, String result) {
                ...
                try {
                    ...
                    SocketIOConnection.this.webSocketClient =  new WebSocketClient(URI.create(sessionUrl), new Listener() {
                            ...
                            try {
                                ...
                                case 6:
                                    ...
                                    for (TimeoutModel timeoutModel : timeoutModels) {
										if (timeoutModel.ack.equalsIgnoreCase(ackParts[0])) {
											timeoutModels.remove(timeoutModel);
											 break;
										}
									}
                                    break;
						...
						@Override
						public void onTimeOutException(Exception error) {
							if (acknowledges.size() > 0) {
								for (TimeoutModel timeoutModel : timeoutModels) {
									 if(acknowledges.containsKey(timeoutModel.ack)){
										 timeoutModel.timeoutCallback.onTimeOutException(timeoutModel.UniqueId,error);
										 acknowledges.remove(timeoutModel.ack);
										 timeoutModels.remove(timeoutModel);
										 break;
									 }
								}
    ...
    private void reportTimeout(final Exception ex) {
        select(null, new SelectCallback() {
            @Override
            public void onSelect(final SocketIOClient client) { 
            	 final TimeoutCallback closed = client.getTimeoutCallback();
                 if (closed != null) {
                     mHandler.post(new Runnable() {
                          @Override
                         public void run() {
                             closed.onTimeOutException(null,ex);
                          }
                     });
                  }
            }
