...
public class SocketIOClient extends EventEmitter {
    ...
    private void emitRaw(int type, String message, Acknowledge acknowledge,String UniqueId , TimeoutCallback callback) {
        connection.emitRaw(type, this, message, acknowledge,UniqueId,callback);
    }
    ...
    public void emit(String name, JSONArray args, Acknowledge acknowledge) {
        ...
        try {
            event.put("name", name);
            event.put("args", args);
            emitRaw(5, event.toString(), acknowledge,null,null);
        } catch (Exception e) {
        }
    }
    
    public void emit(String name, JSONArray args, Acknowledge acknowledge, String UniqueId ,TimeoutCallback callback) {
        final JSONObject event = new JSONObject();
        try {
            event.put("name", name);
            event.put("args", args);
            emitRaw(5, event.toString(), acknowledge,UniqueId,callback);
        } catch (Exception e) {
        }
    }
    ...
    public void emit(final String message, Acknowledge acknowledge) {
        emitRaw(3, message, acknowledge,null,null);
    }

    public void emit(final JSONObject jsonMessage, Acknowledge acknowledge) {
        emitRaw(4, jsonMessage.toString(), acknowledge,null,null);
    }
